<template>
  <div class="new__requests">
              ...
              <tr v-for="item in requests">
                <td>
                  <label class="input-checkbox">
                    <input type="checkbox" class="input-checkbox__input add__comment-checkbox" @click="showComment">
                    <span></span>
                  </label>
                </td>
                <td>REA</td>
                ...
                <td>
                  <div class="table__buttons">
                    ...
                    <router-link :to="'/detail/'+item.id" class="table__button">
    ...
    <CommentModal/>
    <MapModal/>
    <ContentBottom @pagination="setPage" @loadmore="loadMore" @change-per-page="changePerPage" :paginator="paginator"/>
  </div>
</template>
...
export default {
  ...
  created() {
    ...
    this.requests = this.getRequests(this.cur_page);
    this.getItems();
  },
  methods: {
    getItems(page_num = 1) {
      axios.get('/requests/all', {
        params: {
          per_page: this.$store.getters["internalVariables/getPerPage"],
          page: page_num
        }
      }).then(res => {
  ...
  computed: {
    ...mapGetters({
      getRequests: 'newRequests/get_request_page',
      getAppRequests: 'newRequests/get_all_requests'
    }),
  },
  components: {
    ContentBottom,
